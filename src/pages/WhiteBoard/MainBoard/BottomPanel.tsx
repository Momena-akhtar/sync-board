...
const tools: Tool[] = [
    ...
    { 
        ...
        submenuItems: [
            ...
            { name: "Arrow", icon: <ArrowDownLeft size={16} />, key: "arrow" },
        ]
    },
...
const BottomPanel: React.FC<BottomPanelProps> = ({ 
    ...
    return (
        <div 
            className="relative flex justify-center items-center gap-4 bg-transparent backdrop-blur-lg border border-[#454545] font-[Kumbh_Sans] p-4 rounded-[20px]"
                    ...
                    <div key={tool.key} className="relative flex flex-col items-center">
                        <button
                            className={`flex items-center gap-1 cursor-pointer text-white hover:opacity-80 transition-all duration-200 ${
                                activeTool === tool.key || 
                                (tool.submenuItems?.some(item => item.key === activeTool) ?? false) 
                                    ? 'text-blue-400' 
                                    : ''
                            }`}
                            ...
                            {tool.hasSubmenu && (
                                <ChevronDown 
                                    ...
                                    className={`transition-transform duration-200 ${
                                        openSubmenu === tool.key ? 'rotate-180' : ''
                                    }`} 
                                />
                            )}
                        </button>
                                ...
                                {tool.submenuItems?.map((item) => (
                                    <div 
                                        ...
                                        className={`flex items-center gap-2 px-3 py-2 hover:bg-[#2a2a2a] cursor-pointer ${
                                            activeTool === item.key ? 'bg-[#2a2a2a] text-blue-400' : ''
                                        }`}
                                        onClick={() => {
